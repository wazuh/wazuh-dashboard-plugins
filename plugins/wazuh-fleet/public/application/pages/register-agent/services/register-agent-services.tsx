...
export const clusterStatusResponse = async (): Promise<boolean> => {
  const clusterStatus = await getWazuhCore().http.server.request(
    'GET',
    '/cluster/status',
    {},
  );
...
async function getRemoteConfiguration(
  ...
  try {
    ...
    if (clusterStatus) {
      result = await getWazuhCore().http.server.request(
        'GET',
    ...
    if (remote) {
      const remoteFiltered = remote.filter(
        (item: RemoteItem) => item.connection === 'secure',
      );
      ...
      remote.forEach((item: RemoteItem) => {
        ...
        for (const protocol of item.protocol) {
...
async function getAuthConfiguration(node: string, clusterStatus: boolean) {
  ...
  const result = await getWazuhCore().http.server.request(
    'GET',
    authConfigUrl,
    {},
  );
...
function getRemoteProtocol(protocols: Protocol[]) {
  if (protocols.length === 1) {
    ...
    return protocols.includes('TCP') ? 'TCP' : 'UDP';
  }
}
...
export const getNodeIPs = async (): Promise<any> =>
  await getWazuhCore().http.server.request('GET', '/cluster/nodes', {});

/**
 * Get the list of the manager and parse it into a list of options
 */
export const getManagerNode = async (): Promise<any> => {
  const managerNode = await getWazuhCore().http.server.request(
    'GET',
    '/manager/api/config',
    {},
  );
...
export const getMasterNode = (
  ...
  nodeIps.filter(nodeIp => nodeIp.nodetype === 'master');
