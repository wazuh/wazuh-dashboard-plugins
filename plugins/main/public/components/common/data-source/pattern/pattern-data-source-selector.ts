...
export class PatternDataSourceSelector
  implements tDataSourceSelector<PatternDataSource>
{
  // add a map to store locally the data sources
  private dataSources: Map<string, PatternDataSource> = new Map();

  constructor(
    dataSourcesList: PatternDataSource[],
    private repository: tDataSourceRepository<tParsedIndexPattern>,
  ) {
    if (!repository) {
