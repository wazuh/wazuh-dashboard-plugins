...
function createMockedSavedObjectListResponse(
...
describe('PatternDataSourceRepository', () => {
  ...
  it('should return a pattern by id', async () => {
    ...
    expect(result._fields).toEqual(
      JSON.parse(expectedResponse.data.attributes.fields),
    );
  });
  ...
  it('should return an ERROR when the request throw an error', async () => {
    ...
    await expect(repository.get(id)).rejects.toThrow(
      `Error getting index pattern: ${expectedError.message}`,
    );
  });
  ...
  it('should return an ERROR when the response not return a saved object data', async () => {
    ...
    try {
      ...
      expect(error.message).toBe(
        `Error getting index pattern: Index pattern "${id}" not found`,
      );
    }
  });
  ...
  it('should return an ERROR when the request not return and id or a title', async () => {
    ...
    try {
      ...
      expect(error.message).toBe(
        'Error getting index pattern: Invalid index pattern data',
      );
    }
  });
  ...
  it('should return all the index patterns', async () => {
    ...
    result.forEach((item, index) => {
      ...
      expect(item.title).toBe(
        listOfMockedPatterns[index].data.attributes.title,
      );
  ...
  it('should return ERROR when the getAll request throw an error ', async () => {
    ...
    await expect(repository.getAll()).rejects.toThrow(
      `Error getting index patterns: ${expectedError.message}`,
    );
  });
