...
export class SystemInventoryPackagesStatesDataSourceRepository extends PatternDataSourceRepository {
  constructor() {
    super();
  }
  ...
  validate(dataSource): boolean {
    ...
    for (const field of fieldsToCheck) {
      if (
        dataSource[field] &&
        dataSource[field].toLowerCase().includes('inventory-packages')
      ) {
        return true;
      }
    }
