...
export class SystemInventoryHotfixesStatesDataSourceRepository extends PatternDataSourceRepository {
  constructor() {
    super();
  }
  ...
  validate(dataSource): boolean {
    ...
    for (const field of fieldsToCheck) {
      if (
        dataSource[field] &&
        dataSource[field].toLowerCase().includes('inventory-hotfixes')
      ) {
        return true;
      }
    }
