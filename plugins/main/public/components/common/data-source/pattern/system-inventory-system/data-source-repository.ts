...
export class SystemInventorySystemStatesDataSourceRepository extends PatternDataSourceRepository {
  constructor() {
    super();
  }
  ...
  validate(dataSource): boolean {
    ...
    for (const field of fieldsToCheck) {
      if (
        dataSource[field] &&
        dataSource[field].toLowerCase().includes('inventory-system')
      ) {
        return true;
      }
    }
