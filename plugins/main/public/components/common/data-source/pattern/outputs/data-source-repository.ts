...
export class OutputsDataSourceRepository extends PatternDataSourceRepository {
  constructor() {
    super();
  }
  ...
  async get(id: string) {
    ...
    if (this.validate(dataSource)) {
      ...
      throw new Error('Outputs index pattern not found');
    }
  }
  ...
  validate(dataSource): boolean {
    ...
    for (const field of fieldsToCheck) {
      if (
        dataSource[field] &&
        dataSource[field].toLowerCase().includes('outputs')
      ) {
        return true;
      }
    }
