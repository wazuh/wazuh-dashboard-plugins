...
export const ChartDonut = (props: ChartDonutProps) => {
  ...
  const onEnter = () => {
    ...
    tooltipRootRef.current = createRoot(tooltip[0][0]);
  };
  ...
  const onMove = d => {
    ...
    tooltip
      ...
      .style('display', 'block');
    tooltipRootRef.current?.render(<TooltipContent data={d.data} />);
  };

  const onOut = () => {
    tooltipRootRef.current?.unmount();
    tooltipRootRef.current = null;
    d3.select('body > div.wz-chart-tooltip').remove();
  };
  ...
  useEffect(() => {
    ...
    const path = groupWithData.select('path')[0].filter(item => item != null)
      .length
      ? groupWithData.select('path')
      : groupWithData.append('path');
  ...
  return (
    <svg width={size} height={size}>
      <g
        ref={pieRef}
        transform={`translate(${[size / 2, size / 2].join(',')})`}
      />
    </svg>
  );
};
