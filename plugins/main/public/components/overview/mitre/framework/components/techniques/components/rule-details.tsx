...
const getComplianceKey = key => {
  if (key === 'pci') {
    return 'compliance.pci_dss.requirements';
  }
  if (key === 'gdpr') {
    return 'compliance.gdpr.requirements';
  }
  if (key === 'gpg13') {
    return 'rule.gpg13';
  }
  if (key === 'hipaa') {
    return 'compliance.hipaa.requirements';
  }
  if (key === 'nist-800-53') {
    return 'compliance.nist_800_53.requirements';
  }
  if (key === 'mitre') {
