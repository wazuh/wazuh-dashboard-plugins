...
describe('DevToolsMirrors', () => {
  it('renders left/right columns and textareas', () => {
    const { container } = render(
      <DevToolsMirrors onSendRequestButton={() => {}} />,
    );

    expect(
      container.querySelector(`#${EDITOR_MIRRORS.LEFT_COLUMN_ID}`),
    ).toBeTruthy();
    expect(
      container.querySelector(`#${EDITOR_MIRRORS.RIGHT_COLUMN_ID}`),
    ).toBeTruthy();

    expect(container.querySelector(`#${EDITOR_MIRRORS.INPUT_ID}`)).toBeTruthy();
    expect(
      container.querySelector(`#${EDITOR_MIRRORS.OUTPUT_ID}`),
    ).toBeTruthy();
  });
  ...
  it('invokes onSendRequestButton when clicking play icon', () => {
    ...
    const { container } = render(
      <DevToolsMirrors onSendRequestButton={onSend} />,
    );
