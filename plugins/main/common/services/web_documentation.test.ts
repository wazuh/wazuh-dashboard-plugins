...
test(`Generate a web documentation URL to the base URL using to the plugin short version`, () => {
  expect(webDocumentationLink('')).toBe(
    `${DOCUMENTATION_WEB_BASE_URL}/${PLUGIN_VERSION_SHORT}/`,
  );
});

test(`Generate a web documentation URL using a specific version`, () => {
  expect(
    webDocumentationLink('user-manual/agent-enrollment/index.html', '4.7'),
  ).toBe(
    `${DOCUMENTATION_WEB_BASE_URL}/4.7/user-manual/agent-enrollment/index.html`,
  );
});
