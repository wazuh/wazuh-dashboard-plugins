...
function generateRandomLogin() {
  return {
    ...
    tty: [random.choice(['pts/0', 'pts/1', 'pts/2', 'pts/3', 'console', 'rdp-tcp#0', 'rdp-tcp#1', 'services', 'systemd'])],
...
function generateRandomUser() {
  ...
  const names = isService 
    ...
    : ['administrator', 'root', 'john.doe', 'marie.martin', 'admin.local', 'sysadmin', 'developer.user', 'support.tech'];
  
  const fullNames = isService
    ? ['Wazuh Agent Service', 'Backup Service Account', 'System Service', 'Web Service Account']
    : ['System Administrator', 'root', 'John Doe', 'Marie Martin', 'Local Administrator', 'System Administrator', 'Software Developer', 'Technical Support'];
  ...
  const homes = isWindows
    ? ['C:\\Users\\Administrator', 'C:\\Users\\admin.local', 'C:\\Users\\marie.martin', 'C:\\Windows\\ServiceProfiles\\BackupService']
  ...
  return {
    auth_failures: {
      ...
      timestamp: authFailureCount > 0 ? DateFormatter.format(
        Random.date(),
        DateFormatter.DATE_FORMAT.ISO_TIMESTAMP,
      ) : null,
    },
    created: DateFormatter.format(
      Random.date(),
      DateFormatter.DATE_FORMAT.ISO_TIMESTAMP,
    ),
    ...
    last_login: DateFormatter.format(
      Random.date(),
      DateFormatter.DATE_FORMAT.ISO_TIMESTAMP,
    ),
    name: random.choice(names),
    password: {
      expiration_date: passwordStatus === 'locked' ? '1970-01-01T00:00:00.000Z' : DateFormatter.format(
        Random.date(),
        DateFormatter.DATE_FORMAT.ISO_TIMESTAMP,
      ),
      ...
      last_set_time: DateFormatter.format(
        Random.date(),
        DateFormatter.DATE_FORMAT.ISO_TIMESTAMP,
      ),
    ...
    type: userType,
    uid_signed: userId,
    uuid: isWindows ? `S-1-5-21-${random.int(123456789, 987654321)}-${random.int(1001, 1010)}` : `LINUX-${isService ? 'SVC' : userType.toUpperCase()}-${userId}`,
  };
}
