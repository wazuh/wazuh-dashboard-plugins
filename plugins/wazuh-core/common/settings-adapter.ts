...
import { EConfigurationProviders, EpluginSettingType, SettingCategory, TPlugginSettingOptionsObjectOf, TPluginSetting } from "./constants";
...
const schemaMapper = (setting: TPluginSetting ) => {
    ...
    switch (type) {
      case EpluginSettingType.objectOf:
        ...
        const options = setting?.options?.objectOf as TPlugginSettingOptionsObjectOf;
...
export const uiSettingsAdapter = (settings: { [key: string]: TPluginSetting }) : Record<string, UiSettingsParams> => {
  ...
  export const configSettingsAdapter = (settings: { [key: string]: TPluginSetting }) => {
  ...
  export const getConfigSettingsDefinitions = (settings: { [key: string]: TPluginSetting }) => {
    const onlyConfigSettings = getSettingsByType(settings, EConfigurationProviders.INITIALIZER_CONTEXT);
