...
export function getCategorySettingByTitle(categoryTitle: string): any {
  return Object.entries(PLUGIN_SETTINGS_CATEGORIES).find(
    ([key, category]) => category?.title == categoryTitle,
  )?.[1];
}
...
export function getSettingsDefault(): { [key in TPluginSettingKey]: unknown } {
  return Object.entries(PLUGIN_SETTINGS).reduce(
    (accum, [pluginSettingID, pluginSettingConfiguration]) => ({
      ...accum,
 ...
 * @returns
 */
export function getPluginSettingDescription({
  description,
  options,
}: TPluginSetting): string {
  return [
    description,
    ...(options?.select
      ? [
    ...
    ...(options?.file?.recommended?.dimensions
      ...
      : []),
    // File size
    ...(options?.file?.size && typeof options.file.size.minBytes !== 'undefined'
      ? [`Minimum file size: ${formatBytes(options.file.size.minBytes)}.`]
      : []),
    ...(options?.file?.size && typeof options.file.size.maxBytes !== 'undefined'
 ...
 * @returns
 */
export function getCustomizationSetting(
  ...
  if (
    ...
    return typeof configuration[settingKey] !== 'undefined'
      ? resolveEmptySetting(settingKey, configuration[settingKey])
      : defaultValue;
  } else {
    return defaultValue;
  }
