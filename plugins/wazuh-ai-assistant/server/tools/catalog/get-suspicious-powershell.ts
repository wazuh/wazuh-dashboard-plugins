 ...
 * as an analyzed `match` on `wazuh.rule.description` for "powershell" (the standard analyzer lowercases
  ...
  buildRequest(params) {
    ...
    return {
      ...
      body: {
        query: {
          bool: {
            // 5.0 CORRECTNESS FIX: this filtered on `match: {'wazuh.rule.description': 'powershell'}`
            // ALONE, which can NEVER match on Wazuh 5.0 — `wazuh.rule.description` is mapped `keyword`
            ...
            // canonical ATT&CK technique id for PowerShell, the `wazuh.rule.tags` vocabulary, and the
