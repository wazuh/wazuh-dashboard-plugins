...
export const getCriticalVulnerabilitiesTool: ToolDefinition = {
  ...
  buildRequest(params) {
    ...
    return {
      ...
      body: {
        ...
        sort: ['_doc'],
        size: limit,
        // Population-true severity/agent breakdown over the FULL matched set -- see
        // VULN_BREAKDOWN_AGGS's doc comment in common.ts. The severity bucket is degenerate here
        // (this tool always filters to Critical), but the agent-name bucket still discloses every
        // affected agent regardless of `limit` truncation, and attaching the same shared aggs
        // uniformly across all three vulnerability tools keeps behavior consistent regardless of
        // which of the three the model calls.
  ...
  tableSpec: {
    columns: [
      // Column order: identical treatment to its three siblings (get-vulnerabilities.ts et al.)
      // — this is the 4th tool with the same 8-column set, and leaving it un-reordered would
      // show Architecture in the visible 6 while hiding Description and CVSS, the exact ordering
      // the sibling files' comments argue against.
