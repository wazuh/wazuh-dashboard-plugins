...
export const getBruteForceTool: ToolDefinition = {
  spec: {
    ...
    description:
      ...
      'within a time range (MITRE technique T1110 or its rule tags), most recent first. Reports ' +
      'only that such a finding fired -- it does NOT indicate whether Wazuh took any automated ' +
      'action (active response, blocking, quarantine) afterward; no tool covers active-response ' +
      'actions, so say that plainly instead of inferring one from this finding.',
