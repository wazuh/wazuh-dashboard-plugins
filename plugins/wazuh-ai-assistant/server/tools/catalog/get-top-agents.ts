...
export const getTopAgentsTool: ToolDefinition = {
  spec: {
    ...
    description:
      ...
      'per agent ID. The obvious choice for "which agents are noisiest", "top N agents by ' +
      'findings/events", or "most active hosts" questions. Defaults to ranking by rule-matched ' +
      'findings; set index to "events" to rank by raw event/telemetry volume instead.',
  ...
  digest: { sampleColumns: ['key', 'doc_count', 'wazuh.agent.name'] },
};
