...
function fakeLogger() {
  ...
  return {
    debugCalls,
    logger: { debug: (message: string) => debugCalls.push(message) } as any,
  };
}
...
test('validateSuggestDiscoverQueryArgs: accepts a well-formed call', () => {
  ...
  if (result.ok) {
    ...
    assert.equal(result.reason, 'This filter needs a rule id I could not confirm.');
  }
});
...
test('resolveSuggestedDsl: an index outside the allowlist strips to index + time range, never calls _field_caps', async () => {
  ...
  assert.equal(called, false, '_field_caps must never be called for a disallowed index');
...
test('resolveSuggestedDsl: an allowed index with an unknown field strips to index + time range', async () => {
  const { logger } = fakeLogger();
  const context = fakeContext(async () => ({
    body: { fields: {} }, // field_caps reports nothing -- the referenced field does not exist
  }));
  const dsl = {
    bool: {
      filter: [
...
test('resolveSuggestedDsl: a wazuh-states-* index uses state.modified_at for the stripped time range', async () => {
  const { logger } = fakeLogger();
  const context = fakeContext(async () => ({ body: { fields: {} } }));
  ...
  assert.deepEqual(result, {
    bool: {
      filter: [{ range: { 'state.modified_at': { gte: 'now-24h', lte: 'now' } } }],
    },
  });
});
