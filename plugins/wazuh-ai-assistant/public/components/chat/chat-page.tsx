...
interface ChatPageProps {
   ...
   * not this component's rail/flyout) but no state of its own to render it from, since ChatPage is
   * what loads/saves conversations. Paired with the `ref` handle below, which is how that popover
   * actually acts on a selection instead of just displaying one.
   */
...
/**
 * Imperative actions a caller with no inline rail of its own (the sidecar header's popover) can
 * still take on ChatPage's saved conversations — each goes through the SAME interrupt-confirm gate
 * and dedupe-on-already-active guard the inline rail's own click handlers use, so acting through
 * this handle behaves identically to clicking a rail row would have.
 */
...
export const ChatPage = React.forwardRef<ChatPageHandle, ChatPageProps>(
  (
    {
      core,
      providers,
    ...
    const [isRestoringConversation, setIsRestoringConversation] =
      useState(false);
    ...
    // No deps array: a caller (the sidecar header's popover) acting on this handle must never see a
    // stale `handleSelectConversation`/`confirmIfGenerating` closure from an earlier render — these
    // functions are cheap to recreate and this component already re-renders on every keystroke (see
    // `message-list.tsx`'s own doc comment on that), so recomputing here every render costs nothing
    // extra.
    useImperativeHandle(ref, () => ({
      newConversation: () => void confirmIfGenerating(handleNewConversation),
      selectConversation: (id: string) => {
        // Same dedupe the inline rail's own `onSelect` does before its `confirmIfGenerating` call
        // (chat-page.tsx's rail JSX below) — without it, reselecting the conversation already open
        // would ask to confirm an interruption that was never going to happen.
        if (id === activeConversationIdRef.current) {
          return;
        }
