...
# Generate certificates to use with nginx
# Command extrated from https://github.com/wazuh/wazuh-docker/blob/master/production_cluster/nginx/ssl/generate-self-signed-cert.sh
echo "Generate the nginx certificates..."
mkdir -p setup/nginx_certs
openssl req -x509 -batch -nodes -days 365 -newkey rsa:2048 -keyout setup/nginx_certs/key.pem -out setup/nginx_certs/cert.pem
