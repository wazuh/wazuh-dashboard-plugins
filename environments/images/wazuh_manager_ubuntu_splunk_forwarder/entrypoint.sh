...
if [ ! -f $WITNESS_FILE_PATH ]; then
    ...
    /opt/splunkforwarder/bin/splunk add forward-server $INDEXER_IP:9997 -auth admin:admin
...
while sleep 60; do
  service wazuh-manager status > /dev/null 2>&1
  status=$?
  if [ $status -ne 0 ]; then
    echo "Looks like the manager died."
    exit 1
  fi
done