...
logging = logger_manager.setup_logging("pura_email_notification_switch")
...
class EmailNotificationSwitchHandler(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            if method.lower() == "post":
                is_pra_email_enabled = request_body.get("is_pra_email_enabled", None)
                is_jra_email_enabled = request_body.get("is_jra_email_enabled", None)

                email_notification_switch_data = dict()
                email_notification_switch_data["is_pra_email_enabled"] = is_pra_email_enabled
                email_notification_switch_data["is_jra_email_enabled"] = is_jra_email_enabled
    ...
    def post_email_notification_switch(self, request_body):
        ...
        if email_notification_switch_details:
            ...
            endpoint = "{}/{}".format(pra_email_switch_endpoint, email_notification_switch_details['_key'])
        else:
            # there is no entry in kvstore so add new entry
            endpoint = pra_email_switch_endpoint
        if request_body.get("is_pra_email_enabled") is not None:
            email_notification_switch_details["is_pra_email_enabled"] = int(str(request_body["is_pra_email_enabled"]))
        if request_body.get("is_jra_email_enabled") is not None:
            email_notification_switch_details["is_jra_email_enabled"] = int(str(request_body["is_jra_email_enabled"]))
