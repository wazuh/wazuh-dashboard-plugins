...
logging = logger_manager.setup_logging("pura_remote_schedule_scan")
...
class RemoteScheduleScanHandler(PersistentServerConnectionApplication):
    ...
    def post_remote_schedule_scan(self, request_body):
        ...
        if scan_details:
            ...
            endpoint = "{}/{}".format(pra_remote_schedule_scan_endpoint, scan_details['_key'])
        else:
            # there is no entry in kvstore so add new entry
            endpoint = pra_remote_schedule_scan_endpoint
