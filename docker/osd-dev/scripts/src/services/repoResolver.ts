...
export function resolveRepositoryHostPath(
  ...
  if (hostPath) {
    ...
    if (envPaths.siblingRepoHostRoot) {
      candidateBases.push(resolve(envPaths.siblingRepoHostRoot, 'wazuh-dashboard-plugins'));
      candidateBases.push(resolve(envPaths.siblingRepoHostRoot, 'wazuh-dashboard'));
    }

    const resolvedBase = candidateBases.map(stripTrailingSlash).find((base) => {
      if (!base) return false;

      const candidates = [`${base}/plugins/${repoName}`, `${base}/${repoName}`, base];
  ...
  ensureAccessibleHostPath(hostPath, `Repository path for '${repoName}'`, envPaths);
