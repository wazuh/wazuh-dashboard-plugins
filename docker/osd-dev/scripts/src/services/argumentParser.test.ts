...
describe('services/argumentParser', () => {
  ...
  describe('parseArguments', () => {
    ...
    it('parses flags and action/mode/modeVersion correctly', () => {
      const cfg = parseArguments(['-o', '2.9.0', '-d', '2.9.0', '-a', 'rpm', 'up', 'server-local', '4.12.0'], envPaths, logger);
    ...
    it('validates -a allowed values', () => {
      expect(() => parseArguments(['-a', 'rpm'], envPaths, logger)).not.toThrow();
    ...
    it('auto-detects -base when no path provided and sibling root available', () => {
      ...
      const dashContainer = path.join(envPaths.siblingContainerRoot, 'wazuh-dashboard');
    ...
    it('throws ConfigurationError when -base cannot be inferred', () => {
      const noSibling: EnvironmentPaths = { ...envPaths, siblingRepoHostRoot: '' };
      expect(() => parseArguments(['-base', 'up'], noSibling, logger)).toThrow(ConfigurationError);
    });
  });
});
