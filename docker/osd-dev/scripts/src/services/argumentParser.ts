...
export function parseArguments(argv: string[], envPaths: EnvironmentPaths): ScriptConfig {
  ...
  while (i < argv.length) {
    ...
    if (arg === '-o') {
      ...
      if (!['rpm', 'deb', 'without', ''].includes(config.agentsUp)) {
        throw new DevScriptError("Invalid value for -a option. Allowed values are 'rpm', 'deb', 'without', or an empty string.");
      }
      i++;
    } else if (arg === '-r') {
      const repoSpec = argv[++i];
      if (!repoSpec.includes('=')) {
        throw new DevScriptError(`Invalid repository specification '${repoSpec}'. Expected format repo=/absolute/path.`);
      }
      const [repoName, repoPath] = repoSpec.split('=');
      if (!repoName || !repoPath) {
        throw new DevScriptError(`Invalid repository specification '${repoSpec}'. Expected format repo=/absolute/path.`);
      }
      config.userRepositories.push({ name: repoName, path: repoPath.replace(/\/$/, '') });
      i++;
      ...
      if (i < argv.length) {
        throw new DevScriptError(`Unexpected arguments: ${argv.slice(i).join(' ')}`);
      }
      break;
    }
  }
  ...
  if (config.useDashboardFromSource && !config.dashboardBase) {
    ...
    if (!hostRoot) {
      throw new DevScriptError('Cannot infer dashboard base path automatically. Provide an absolute path to -base.');
    }
    ...
    if (!containerCandidate) {
      throw new DevScriptError('Unable to locate wazuh-dashboard automatically. Provide an absolute path to -base.');
    }
  ...
  if (config.useDashboardFromSource) {
    if (!config.dashboardBase || !config.dashboardBase.startsWith('/')) {
      throw new DevScriptError('The -base option requires an absolute path to the wazuh-dashboard repository.');
    }
    ensureAccessibleHostPath(config.dashboardBase, 'Dashboard base path', envPaths);
  }

  if (!config.pluginsRoot && envPaths.currentRepoHostRoot) {
    config.pluginsRoot = stripTrailingSlash(resolve(envPaths.currentRepoHostRoot, 'plugins'));
  }
