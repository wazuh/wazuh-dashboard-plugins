...
export function getEnvironmentPaths(): EnvironmentPaths {
  const currentRepoContainerRoot = stripTrailingSlash(
    process.env.WDP_CONTAINER_ROOT || '/wdp',
  );
  const siblingContainerRoot = stripTrailingSlash(
    process.env.SIBLING_CONTAINER_ROOT || '/sibling',
  );
  const currentRepoHostRoot = stripTrailingSlash(
    process.env.CURRENT_REPO_HOST_ROOT || '',
  );
  const siblingRepoHostRoot = stripTrailingSlash(
    process.env.SIBLING_REPO_HOST_ROOT || '',
  );
  ...
  const createNetworksScriptPath = resolve(
    currentRepoContainerRoot,
    'docker',
    'scripts',
