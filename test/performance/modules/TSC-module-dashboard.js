...
module.exports = async function (context, commands) {
  ...
  try {
    ...
    await commands.measure.start('TSC module - Dashboard')
    ...
    logger('--- TSC requirements ---')
    await commands.wait.bySelector('#moduleDashboard .euiFlexItem.h-100 #Wazuh-App-Overview-TSC-requirements .visLib.visLib--legend-right', WAIT_TIMEOUT)

    logger('--- Top 10 agents by alerts number ---')
    await commands.wait.bySelector('#moduleDashboard .euiFlexItem.h-100 #Wazuh-App-Overview-TSC-Agents .visLib.visLib--legend-right', WAIT_TIMEOUT)
    
    logger('--- Top requirements over time ---')
    await commands.wait.bySelector('#moduleDashboard .euiFlexItem.h-100 #Wazuh-App-Overview-TSC-Requirements-over-time .visLib.visLib--legend-right', WAIT_TIMEOUT)
    
    logger('--- Last alerts ---')
    await commands.wait.bySelector('#moduleDashboard .euiFlexItem.h-100 #Wazuh-App-Overview-TSC-Requirements-Agents-heatmap .visLib.visLib--legend-right', WAIT_TIMEOUT)

    logger('--- Requirements by agent ---')
    await commands.wait.bySelector('#moduleDashboard .euiFlexItem.h-100 #Wazuh-App-Overview-TSC-Requirements-by-agent .visLib.visLib--legend-right', WAIT_TIMEOUT)
