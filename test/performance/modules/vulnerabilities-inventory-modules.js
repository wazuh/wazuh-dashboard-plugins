...
module.exports = async function (context, commands) {
  ...
  try {
    ...
    logger('--- VULNERABILITIE INVENTORY DETAILS ---')
    await commands.wait.bySelector('.euiFlexItem .statWithLink.wz-user-select-none', WAIT_TIMEOUT)
    
    logger('--- VULNERABILITIE INVENTORY SUMMARY ---')
    await commands.wait.bySelector('.euiFlexGroup .euiFlexItem:nth-child(3) .euiCard__content .euiFlexItem', WAIT_TIMEOUT)

    logger('--- VULNERABILITIE TABLE ---')
    await commands.wait.bySelector('.wz-inventory tbody tr', WAIT_TIMEOUT)
