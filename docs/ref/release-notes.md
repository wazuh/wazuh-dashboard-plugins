...
- New "[Active Response][active-response-module]" module [#34606](https://github.com/wazuh/wazuh/issues/34606).
  ...
  - New "Add Active Response" action in Alerting monitor trigger configuration — only active response channels are shown, independently of standard notifications [wazuh/wazuh-dashboard-alerting#6](https://github.com/wazuh/wazuh-dashboard-alerting/issues/6).
...
- Dashboards redesigned as saved objects — inline dashboards from 4.x are not compatible with the new renderer. Custom dashboards must be exported before migration and re-imported afterwards [#7842](https://github.com/wazuh/wazuh-dashboard-plugins/pull/7842).
...
- Cluster mode mandatory for all installations [#7688](https://github.com/wazuh/wazuh-dashboard-plugins/issues/7688) [wazuh/wazuh#31295](https://github.com/wazuh/wazuh/issues/31295) — the distinction between cluster and manager mode is removed; the dashboard always operates in cluster mode, including single-node deployments.
  - All filters and search queries use `cluster.name` instead of `manager.name`.
...
- Legacy reporting application removed from the dashboard plugins. PDF and CSV generation is now handled by the Wazuh Reporting fork bundled with Wazuh Dashboard packages [#7813](https://github.com/wazuh/wazuh-dashboard-plugins/issues/7813) [wazuh/wazuh-indexer-reporting#45](https://github.com/wazuh/wazuh-indexer-reporting/issues/45).
