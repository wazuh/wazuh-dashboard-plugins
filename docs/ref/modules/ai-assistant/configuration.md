...
| `wazuh_ai_assistant.outOfCreditsMessage` | unset   | Replaces the chat error shown when a provider reports an out-of-credits condition (an HTTP 402, or a known billing message such as Anthropic's low credit balance or an OpenAI-compatible `insufficient_quota`) — set it to point users at your own credits or plan-upgrade flow instead of the provider's raw text. Transient rate limits are unaffected. A Markdown link (`[label](https://...)`) renders as a link that opens in a new tab; raw HTML is stripped. One value per environment (not per provider), so it is set here rather than from **AI Assistant → Settings**. Unset leaves the provider's own message unchanged. |
