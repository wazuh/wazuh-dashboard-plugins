...
| `index-pattern:metrics-normalization`               | Validate (create if possible) the existence of a compatible index pattern for server metrics                                                                                                                                                                                    |
...
| `index-pattern:states-fim`                          | Validate (create if possible) the existence of a compatible index pattern for FIM                                                                                                                                                                                               |
...
```log
server    log   [10:52:31.480] [info][healthcheck] Enabled checks [46]: [integrations:default-notifications-channels,server-api:connection-compatibility,server-api:run-as,saved-objects:dashboards,index-pattern:metrics-agents,index-pattern:metrics-comms, index-pattern:metrics-normalization,index-pattern:states-vulnerabilities,index-pattern:states-inventory,index-pattern:states-inventory-groups,index-pattern:states-inventory-hardware,index-pattern:states-inventory-hotfixes,index-pattern:states-inventory-interfaces,index-pattern:states-inventory-networks,index-pattern:states-inventory-packages,index-pattern:states-inventory-ports,index-pattern:states-inventory-processes,index-pattern:states-inventory-protocols,index-pattern:states-inventory-system,index-pattern:states-inventory-users,index-pattern:states-inventory-services,index-pattern:states-inventory-browser-extensions,index-pattern:states-fim,index-pattern:states-fim-files,index-pattern:states-fim-registry-keys,index-pattern:states-fim-registry-values,index-pattern:states-sca,index-pattern:events-system-activity,index-pattern:events-security,index-pattern:events-access-management,index-pattern:events-applications,index-pattern:events-other,index-pattern:events-network-activity,index-pattern:events-cloud-services,index-pattern:events,index-pattern:active-responses,index-pattern:events-unclassified,index-pattern:events-raw,index-pattern:findings-system-activity,index-pattern:findings-security,index-pattern:findings-access-management,index-pattern:findings-applications,index-pattern:findings-other,index-pattern:findings-network-activity,index-pattern:findings-cloud-services,index-pattern:findings-unclassified,index-pattern:findings]
```
