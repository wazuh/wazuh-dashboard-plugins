...
| `server-api:run_as`                                                 | Validate that each server API host has `run_as` enabled and the API user has permission to use it; warns when the host disables `run_as` or the user permission is not granted (the user permission maps to `allow_run_as` in the Wazuh API) |
