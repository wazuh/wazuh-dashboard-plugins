...
| `integrations:default-integrations` | Validate the existence of the default Notifications channels and Alerting monitors; creates them if missing (monitors are only created if their corresponding channels exist). See Notifications and Alerting for details. |
