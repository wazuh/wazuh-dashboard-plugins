...
export default [
  {
    ...
    _source: {
      ...
      visState:
        `{"title":"Wazuh App Statistics remoted Recv bytes","type":"timelion","params":{"expression":".es(index=wazuh-stati*,timefield=timestamp,metric=avg:remoted.recv_bytes, q='*').label(recv_bytes),.es(timefield=timestamp,metric=avg:remoted.recv_bytes, q='*').trend().label(Trend).lines(width=1.5)","interval":"auto"},"aggs":[]}`,
      uiStateJSON: '{}',
      description: '',
      version: 1,
