...
export class WazuhReportingCtrl {
   ...
   */
  async report(context: RequestHandlerContext, request: KibanaRequest, response: KibanaResponseFactory) {
    try {
      ...
      if (request.body && request.body.array) {
        ...
        const { name, tab, section, isAgents, browserTimezone, searchBar, filters } = body;
        const apiId = headers.id || false;
        const pattern = headers.pattern || false;
        const from = (body.time || {}).from || false;
        const to = (body.time || {}).to || false;
        let kfilters = request.body.filters;
        ...
        // removed because it is validated in the route
        // if (!tab)
        //   throw new Error(
        ...
        if (isGroupConfig) {
          ...
          const g_id = filters[0].group;
          kfilters = [];
          const enabledComponents = request.body.components;
          printer.addContent({
        ...
        if (isAgentConfig) {
          ...
          const enabledComponents = request.payload.components;
          const a_id = kfilters[0].agent;
          let wmodules = {};
          try {
            wmodules = await context.wazuh.api.client.asCurrentUser.request(
              'GET',
          ...
          kfilters = [];
          await this.renderHeader(context, printer, tab, tab, a_id, apiId);
          let idxComponent = 0;
            ...
            for (let section of config.sections) {
              if (
                ...
                for (let conf of configs) {
                  ...
                  try {
                    if (!conf['name']) {
                      data = await context.wazuh.api.client.asCurrentUser.request(
                        'GET',
        ...
        if (request.body.time || isSycollector) {
        ...
        if (!isSycollector && !isAgentConfig && !isGroupConfig) {
          printer.addVisualizations(request.body.array, isAgents, tab);
        }
        if (isSycollector) {
