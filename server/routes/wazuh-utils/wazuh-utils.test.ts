...
`;
  ...
  it.each`
    testTitle | settings | responseStatusCode | responseBodyMessage
    ${'Update the plugin configuration'} | ${{pattern: 'test-alerts-groupA-*'}} | ${200} | ${null}
    ${'Update the plugin configuration'} | ${{pattern: 'test-alerts-groupA-*', 'logs.level': 'debug'}} | ${200} | ${null}
    ${'Bad request, wrong value type.'} | ${{pattern: 5}} | ${400} | ${"[request body.pattern]: expected value of type [string] but got [number]"}
    ${'Bad request, unknown setting'} | ${{'unknown.setting': 'test-alerts-groupA-*'}} | ${400} | ${"[request body.unknown.setting]: definition for this key is missing"}
    ${'Bad request, unknown setting'} | ${{'unknown.setting': 'test-alerts-groupA-*','logs.level': 'debug'}} | ${400} | ${"[request body.unknown.setting]: definition for this key is missing"}
    ${'Bad request, unknown setting'} | ${{'cron.statistics.apis': [0, 'test']}} | ${400} | ${"[request body.cron.statistics.apis.0]: expected value of type [string] but got [number]"}
