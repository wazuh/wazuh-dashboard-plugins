	...
	const formats = {
		...
		"bulk-api": {
			...
			run: (createAlerts, configuration) => {
				...
				return createAlerts(configuration).map(alert => `{"index": {"_index": "${configuration.index}"}}\n${JSON.stringify(alert)}\n`).join("");
			}
		}
	};
...
Options:
  ...
  --format <format>                                   Set the format. Default: ${defaultConfiguration.format}.
    <format> is one of:
${Object.keys(formats).sort().map(formatID => `      ${formatID}: ${formats[formatID].description}`).join('\n')}
  --module-<module-id>                                Enable the alerts generation of this module.
    <module-id> is one of: ${Object.keys(alertModules).sort().join(', ')}.  You can add multiple flags to enable more modules.
  --param-cluster-name <cluster-name>                 Set the cluster name. Default: ${defaultConfiguration.alertGenerationParams.cluster.name}.
  --param-cluster-node <cluster-node>                 Set the cluster node. Default: ${defaultConfiguration.alertGenerationParams.cluster.node}.
  --param-manager-name <manager-name>                 Set the manager name. Default: ${defaultConfiguration.alertGenerationParams.manager.name}.
  --output <output>                                   Set the output. Default: ${defaultConfiguration.output}.
    <output> is one of:
${Object.keys(outputs).sort().map(outputID => `      ${outputID}: ${outputs[outputID].description}`).join('\n')}
`)
	};
	...
	try {
		...
		while (consoleInputParameters.length) {
			...
			switch (parameter) {
				...
				case '--size':
					...
					const size = typeof consoleInputParameters[0] === "string" ? Number(consoleInputParameters[0]) : null;
					if (size && !isNaN(size)) {
						configuration.size = size;
						consoleInputParameters.splice(0, 1);
					} else {
						console.error(`Size value ${consoleInputParameters[0]} is not a number.`);
						process.exit(1);
					};
					break;
				...
				case '--index':
					...
					const index = typeof consoleInputParameters[0] === "string" ? consoleInputParameters[0] : null;
					if (index) {
						configuration.index = index;
						consoleInputParameters.splice(0, 1);
					} else {
						console.error(`Index value ${consoleInputParameters[0]} is not defined.`);
						process.exit(1);
					}
					break;
				case '--param-manager-name':
					// Set the index name used by some formats
					const managerName = typeof consoleInputParameters[0] === "string" ? consoleInputParameters[0] : null;
					if (managerName) {
						...
						console.error(`Manager name value ${consoleInputParameters[0]} is not defined.`);
						process.exit(1);
					}
					break;
				case '--param-cluster-name':
					// Set the index name used by some formats
					const clusterName = typeof consoleInputParameters[0] === "string" ? consoleInputParameters[0] : null;
					if (clusterName) {
						configuration.alertGenerationParams = {
							...configuration.alertGenerationParams,
							cluster: {
						...
						console.error(`Cluster name value ${consoleInputParameters[0]} is not defined.`);
						process.exit(1);
					}
					break;
				case '--param-cluster-node':
					// Set the index name used by some formats
					const clusterNode = typeof consoleInputParameters[0] === "string" ? consoleInputParameters[0] : null;
					if (clusterNode) {
						configuration.alertGenerationParams = {
							...configuration.alertGenerationParams,
							cluster: {
						...
						console.error(`Cluster node value ${consoleInputParameters[0]} is not defined.`);
						process.exit(1);
					}
					break;
				case '--format':
					// Set the format of alerts
					const format = typeof consoleInputParameters[0] === "string" && formats[consoleInputParameters[0]] ? consoleInputParameters[0] : null;
					if (format) {
						configuration.format = format;
						consoleInputParameters.splice(0, 1);
					} else {
						console.error(`Format value ${consoleInputParameters[0]} is not a allowed.Allowed values: ${Object.keys(formats).join(", ")}`);
						process.exit(1);
					}
					break;
				case '--output':
					// Set the output
					const output = typeof consoleInputParameters[0] === "string" && outputs[consoleInputParameters[0]] ? consoleInputParameters[0] : null;
					if (output) {
						configuration.output = output;
						consoleInputParameters.splice(0, 1);
					} else {
						console.error(`Output value ${consoleInputParameters[0]} is not a allowed. Allowed values: ${Object.keys(outputs).join(", ")}`);
						process.exit(1);
					}
					break;
				default: { };
			...
			const enabledCategoryModule = parameter.match(/--category-module-([A-Za-z\-]+)/);
			const categoryModuleID = enabledCategoryModule ? enabledCategoryModule[1] : null;
			if (enabledCategoryModule && categoryAlertModules[categoryModuleID]) {
				categoryAlertModules[categoryModuleID].forEach(moduleID => configuration.modules[moduleID] = { ...alertModules[moduleID] });
			};
		};
		...
		function createAlerts(configuration) {
			...
			return Object.keys(configuration.modules)
				.reduce((accum, module) => [...accum, ...generateAlerts({...alertModules[module], ...configuration.alertGenerationParams}, configuration.size)]
				, []);
		};

		// Output
		outputs[configuration.output].run( // Run the selected output
