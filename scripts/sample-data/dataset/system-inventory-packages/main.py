...
default_count='10000'
default_index_name='wazuh-states-system-inventory-packages-sample'
...
def generate_random_package():
  ...
  return {
    'name': package_name,
    'version': f'v{random.randint(0, 9)}-stable',
    'vendor': f'vendor{random.randint(0, 255)}',
    'install_time': generate_random_date(),
    'location': f'/path/to/{package_name}',
    'architecture': random.choice(['x86_64', 'arm64', 'amd64']),
    'groups': f'group{random.randint(0, 255)}',
    'description': f'Description for {package_name} package.',
  }
