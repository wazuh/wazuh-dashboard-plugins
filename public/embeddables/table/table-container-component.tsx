...
export function TableContainerComponentInner({ embeddable, input, embeddableServices }: Props) {
  return (
    <div>
      <h2 data-test-subj="tableContainerTitle">{embeddable.getTitle()}</h2>
      <EuiSpacer size="l" />
      {renderTable(embeddable, input.panels, embeddableServices)}
    </div>
  );
}
...
export const TableContainerComponent = withEmbeddableSubscription<
  ContainerInput,
  ContainerOutput,
  IContainer,
  { embeddableServices: EmbeddableStart }
>(TableContainerComponentInner);
