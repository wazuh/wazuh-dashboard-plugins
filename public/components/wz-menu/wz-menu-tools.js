...
class WzMenuTools extends Component {
  ...
  avaibleRenderSettings() {
    return [
      this.createItem({ id: 'devTools', text: 'Dev Console' }),
      this.createItem({ id: 'logtest', text: 'Logtest' }),
    ]
  }
  ...
  createItem = (item, data = {}) => {
    ...
    return {
      ...
      id: item.id,
      name: item.text,
      isSelected: window.location.href.includes('/tools') && this.props.state.selected_tools_section === item.id,
      onClick: () => { },
