...
export class AgentInfo extends Component {
    ...
    const stats = items.map(item => {
      ...
      return (
        <EuiFlexItem key={item.description} style={item.style || null}>
          <WzStat
            title={
              item.description === i18n.translate('components.welcome.agentInfo.itemDescription.groups', { defaultMessage: 'Groups' }) ? (
                <GroupTruncate
                  ...
                  agent={this.props.agent}
                  {...this.props} />
              ) : item.description === i18n.translate('components.welcome.agentInfo.itemDescription.operatingSystem', { defaultMessage: 'Operating system' }) ? (
                this.addTextPlatformRender(this.props.agent, item.style)
              ) : item.description === i18n.translate('components.welcome.agentInfo.itemDescription.status', { defaultMessage: 'Status' }) ? (
  ...
  render() {
    ...
    let arrayStats;
    const id = i18n.translate('components.welcome.agentInfo.stats.id',{
      defaultMessage: 'ID'
    })
    const status = i18n.translate('components.welcome.agentInfo.stats.status',{
      defaultMessage: 'Status'
    })
    const version = i18n.translate('components.welcome.agentInfo.stats.version',{
      defaultMessage: 'Version'
    })
    const ip = i18n.translate('components.welcome.agentInfo.stats.ip',{
      defaultMessage: 'IP'
    })

    const groups = i18n.translate('components.welcome.agentInfo.stats.groups', {           defaultMessage: 'Groups'
    })
