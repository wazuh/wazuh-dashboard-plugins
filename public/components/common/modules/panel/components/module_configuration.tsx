...
export const PanelModuleConfiguration: FunctionalComponent<{ h: string }> =
      ...
      const asyncAction = useAsyncAction(async () => {
        try {
          if (agent.id) {
            ...
            const custerStatusResponse = await WzRequest.apiReq(
              'GET',
              '/cluster/status',
              {},
            );
            if (
              custerStatusResponse.data.data.enabled === 'yes' &&
              custerStatusResponse.data.data.running === 'yes'
            ) {
              // Cluster mode
              ...
              const nodeConfigurations = nodesConfigurationResponses.filter(
      ...
      return (
        <>
          <EuiFlexGroup>
            ...
            <EuiFlexItem>
              <EuiTitle size='s'>
                <h4 className='module-panel-configuration-title'>
                  {moduleTitle}
                </h4>
              </EuiTitle>
              <EuiTitle size='xs'>
          ...
          <EuiFlexGroup direction='column'>
            <EuiFlexItem>
              <ConfigurationWrapper
                configurations={asyncAction.data}
                settings={settings}
      ...
      {configurations.length === 1 ? (
        ...
        configurations
          .map((configuration, index) => (
            <EuiAccordion
              id={`module_configuration_${configuration.name}_${index}`}
              key={`module_configuration_${configuration.name}_${index}`}
              buttonContent={renderEntityTitle(
          ...
          .reduce(
            (prev, cur) => [prev, <div style={{ marginTop: '8px' }} />, cur],
            [],
          )
      )}
    </div>
