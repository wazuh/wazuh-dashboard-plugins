...
export const WzAgentSelector = props => {
  ...
  const fetchData = async (props) => {
    ...
    const agentsResponse = await WzRequest.apiReq('GET', '/agents', {params: filter});
    const items = (((agentsResponse || {}).data || {}).data || {}).affected_items || [];
    const totalItems = (((agentsResponse || {}).data || {}).data || {}).total_affected_items || 0;
...
export const WzGroupSelector = props => {
  ...
  const fetchData = async (props) => {
    const filter = buildFilter(props);
    const groupsResponse = await WzRequest.apiReq('GET', '/groups', {params: filter});
    const items = (((groupsResponse || {}).data || {}).data || {}).affected_items || [];
    const totalItems = (((groupsResponse || {}).data || {}).data || {}).total_affected_items || 0;
