...
export default {
  ...
  api_component: 'logcollector',
  api_configuration: 'localfile',
          ...
          options: [
            {
              ...
              description: 'Set the format of the log to be read.',
              type: 'input',
              required: true,
              placeholder: 'audit',
              default_value: 'audit',
            ...
            },
            {
              ...
              display_name: 'Only future events',
  ...
  mapAgentConfigurationAPIResponse(config){
    return {
      ...(config.localfile ? {
        localfile: config.localfile.filter(localfile => localfile.logformat === 'audit').map(localfile => ({
          ...localfile,
          ...(localfile.logformat ? { log_format: localfile.logformat } : {}),
          ...(localfile.target ? { target: localfile.target.join(',') } : {}),
          ...(localfile.out_format && localfile.out_format[0] ? { out_format: {
            '@': {
              ...(localfile.out_format[0].target ? {target: localfile.out_format[0].target} : {}),
            },
            ...(localfile.out_format[0].format ? {'#': localfile.out_format[0].format} : {}),
          } } : {}),
        }))
      } : {})
    }
