...
export function ContextMenu(props) {
  ...
  const button = (
    <EuiButtonEmpty color='text' size='xs' onClick={() => setIsOpen(!isOpen)}>
      <strong>{conjuntion && conjuntion}</strong> {field} {operators[operator]}{' '}
      {value}
    </EuiButtonEmpty>
  );
  return (
...
function EditFilter(props) {
  ...
  return (
    <EuiForm className='globalFilterItem__editorForm'>
      {conjuntion && EditFilterConjuntion(conjuntion, setConjuntion)}
...
async function updateSuggestsValues(
  suggest: any,
  value: any,
  setSuggetsValues: React.Dispatch<React.SetStateAction<never[]>>,
) {
...
function EditFilterConjuntion(
  ...
  return (
    <EuiFormRow label='Conjuntion'>
      <EuiButtonGroup
        legend='Conjunction'
        options={conjuntionOptions}
        idSelected={`conjuntion-${conjuntion.trim()}`}
        onChange={() =>
          setConjuntion(/and/gi.test(conjuntion) ? ' OR ' : ' AND ')
        }
      />
    </EuiFormRow>
  );
