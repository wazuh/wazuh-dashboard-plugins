...
export const RolesMappingEdit = ({ rule, closeFlyout, rolesEquivalences, roles, internalUsers, onSave }) => {
    ...
    <EuiFlyout
      ...
      <EuiFlyoutBody>
        ...
        <EuiFlexGroup style={{ padding: "0px 24px 24px 24px" }}>
          <EuiFlexItem>
            <RuleEditor save={(rule) => editRule(rule)} initialRule={rule.rule} isLoading={isLoading} isReserved={rule.id < 3} internalUsers={internalUsers}></RuleEditor>
          </EuiFlexItem>
        </EuiFlexGroup>
      </EuiFlyoutBody>
    </EuiFlyout>
