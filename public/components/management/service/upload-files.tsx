...
export class UploadFiles extends Component {
   ...
   */
  renderWarning(title) {
    return <EuiCallOut size="s" title={title} color="warning" iconType="iInCircle" />;
  }
  ...
  render() {
    const button = (
      <WzButtonPermissions
        buttonType="empty"
        permissions={[
          { action: 'manager:upload_file', resource: `file:path:/etc/${this.props.msg}` },
        ]}
        ...
        <div style={{ width: '300px' }}>
          ...
          {this.state.files.length > 0 &&
            ...
            this.checkValidFileExtensions() > 0 && (
              <Fragment>
                <EuiFlexItem>
                  {(!this.state.uploadErrors && this.renderFiles()) || this.renderErrors()}
                </EuiFlexItem>
          ...
          {!this.checkValidFileExtensions() > 0 && (
            <Fragment>{this.renderWarning('The files extensions are not valid.')}</Fragment>
          )}
        </div>
      </EuiPopover>
    );
