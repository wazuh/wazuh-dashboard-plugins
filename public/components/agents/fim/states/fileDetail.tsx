      ...
      if (!item.onlyLinux || (item.onlyLinux && this.props.agent.agentPlatform !== 'windows'))
        return (
          <EuiFlexItem key={idx}>
            <EuiStat
              title={
                <EuiText className={item.checksum ? "detail-value detail-value-checksum" : "detail-value"}>
                {
                  !link ? value :
                  <EuiLink
                  onClick={async (event) => {
