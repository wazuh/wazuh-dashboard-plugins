...
}Memory: {syscollector.isLoading ? <EuiLoadingSpinner size="s" /> : <strong>{ (syscollector.data.hardware.ram.total / 1024).toFixed(2)  } {
...
} {syscollector.isLoading ? <EuiLoadingSpinner size="s" /> : <strong>{syscollector.data.os.architecture}</strong>}</EuiText>
                </EuiFlexItem>
                <EuiFlexItem grow={false}>
                    <EuiText>{
...
} {syscollector.isLoading ? <EuiLoadingSpinner size="s" /> : <strong>{offsetTimestamp('',syscollector.data.os.scan.time)}</strong>}</EuiText>
                </EuiFlexItem>
            </EuiFlexGroup>
        </EuiPanel>
        );
