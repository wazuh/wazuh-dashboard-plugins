        ...
        <EuiFlexItem grow={2} style={{ marginRight: 4, marginTop: 0 }}>
          <EuiPanel paddingSize="m" style={{ margin: '12px 16px 12px 16px' }}>
            <TableWzAPI
              ...
              endpoint={`/syscollector/${agent.id}/netiface?select=${netifaceColumns.map(({field}) => field).join(',')}`}
              searchTable
              downloadCsv
              showReload
        ...
        <EuiFlexItem grow={2} style={{ marginLeft: 4, marginTop: 0 }}>
          <EuiPanel paddingSize="m" style={{ margin: '12px 16px 12px 16px' }}>
            <TableWzAPI
              ...
              endpoint={`/syscollector/${agent.id}/ports?select=${portsColumns[soPlatform].map(({field}) => field).join(',')}`}
              searchTable
              downloadCsv
              showReload
              tablePageSizeOptions={[10, 25, 50, 100]}
        ...
        <EuiFlexItem grow={3} style={{ marginRight: 4 }}>
          <EuiPanel paddingSize="m" style={{ margin: '12px 16px 12px 16px' }}>
            <TableWzAPI
              ...
              endpoint={`/syscollector/${agent.id}/netaddr?select=${netaddrColumns.map(({field}) => field).join(',')}`}
              searchTable
              downloadCsv
              showReload
              tablePageSizeOptions={[10, 25, 50, 100]}
          ...
          <EuiFlexItem grow={1} style={{ marginLeft: 4 }}>
            <EuiPanel paddingSize="m" style={{ margin: '12px 16px 12px 16px' }}>
              <TableWzAPI
                ...
                endpoint={`/syscollector/${agent.id}/hotfixes?select=${windowsUpdatesColumns.map(({field}) => field).join(',')}`}
                searchTable
                downloadCsv
                showReload
                tablePageSizeOptions={[10, 25, 50, 100]}
        ...
        <EuiFlexItem>
          <EuiPanel paddingSize="m" style={{ margin: '12px 16px 12px 16px' }}>
            <TableWzAPI
              ...
              endpoint={`/syscollector/${agent.id}/packages?select=${packagesColumns[soPlatform].map(({field}) => field).join(',')}`}
              searchTable
              downloadCsv
              showReload
              tablePageSizeOptions={[10, 25, 50, 100]}
        ...
        <EuiFlexItem>
          <EuiPanel paddingSize="m" style={{ margin: '12px 16px 12px 16px' }}>
            <TableWzAPI
              ...
              endpoint={`/syscollector/${agent.id}/processes?select=${processColumns[soPlatform].map(({field}) => field).join(',')}`}
              searchTable
              downloadCsv
              showReload
              tablePageSizeOptions={[10, 25, 50, 100]}
