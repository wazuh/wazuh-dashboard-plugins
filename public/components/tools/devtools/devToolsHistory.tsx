...
export function DevToolsHistory({ localStorage, closeHistory, addRequest }) {
  ...
  useEffect(() => {
    ...
    if (initialReq) setSelectedRequest(localStorage[initialReq[0]]);
  }, []);

  const getHistoryFromLocalStorageOrdered = () => {
    const wazuhHistoryElems = Object.keys(localStorage)
      .filter(x => x.startsWith('wazuh:history.elem_'))
      .flatMap(x => {
    ...
    while (wazuhHistoryElems.length > 150) {
      // we only maintain the last 150 requests
  ...
  const showHistoryList = () => {
    ...
    const elements = wazuhElements.map(x => {
      ...
      return (
        <li
          onMouseEnter={() => {
            setHoverRequest(currentElement);
          }}
          onMouseLeave={() => {
            setHoverRequest(false);
          }}
          onClick={() => {
            setSelectedRequest(currentElement);
            tryRequest(isActive, currentElement);
          }}
          className={
            isActive
        ...
        >
          ...
          <EuiIcon style={{ float: 'right' }} type='arrowRight' />{' '}
        </li>
      );
    });

    return wazuhElements.length ? (
      <ul className='history-list'>{elements}</ul>
    ) : (
      <></>
    );
  };
  ...
  const showOutputEditor = () => {
    return (
      <EuiCodeEditor
        ...
        value={
          hoverRequest
            ? formatReq(hoverRequest)
            : selectedRequest
            ? formatReq(selectedRequest)
  ...
  return (
    <EuiPanel paddingSize='s' style={{ margin: '0px 8px' }}>
      ...
      <EuiFlexGroup>
        <EuiFlexItem grow={2}>{showHistoryList()}</EuiFlexItem>
