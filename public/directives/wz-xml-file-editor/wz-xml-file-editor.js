  ...
  return {
    ...
    controller(
      ...
      const saveFile = async params => {
        let close = true;
        const warnMsg = 'File has been updated, but there are errors in the configuration';
        try {
          const text = $scope.xmlCodeBox.getValue();
          const xml = replaceIllegalXML(text);
          if (params.group) {
            await groupHandler.sendConfiguration(params.group, xml);
            try { await validateAfterSent() } catch (err) { params.showRestartManager = 'warn'; close = false }
            const msg = 'Success. Group has been updated';
            params.showRestartManager
              ? params.showRestartManager !== 'warn' ?
                showRestartDialog(msg, params.showRestartManager) : errorHandler.handle(warnMsg, '', true)
            ...
            params.showRestartManager
              ? params.showRestartManager !== 'warn' ?
                showRestartDialog(msg, params.node) : errorHandler.handle(warnMsg, '', true)
              : errorHandler.info(msg, '');
