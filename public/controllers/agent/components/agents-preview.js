...
import {
  withErrorBoundary,
  withGlobalBreadcrumb, withMyErrorBoundary,
  withReduxProvider,
  ...
  withUserAuthorizationPrompt([[{action: 'agent:read', resource: 'agent:id:*'},{action: 'agent:read', resource: 'agent:group:*'}]])
    ...
    buttonComponent = () => {
      ...
      return (
        <>
          <EuiLoadingChart />
          <EuiButton id={'12345b'} onClick={clickError}>
            ERROR CLICK
          </EuiButton>
          {this.state.componentError ? <>{this.buttonError()}</> : <></>}
        </>
      );
    };

    render() {
      const ComponentWithErrorBoundary = withErrorBoundary(this.buttonComponent);
      ...
      return (
        <EuiPage>
          ...
          <EuiFlexItem>
            <EuiFlexGroup
              style={{ marginTop: 0 }}
              className="agents-evolution-visualization-group"
            >
              {(this.state.loading && (
                <EuiFlexItem>
                  <EuiLoadingChart style={{ margin: '75px auto' }} size="xl" />
                </EuiFlexItem>
              )) || (
                <Fragment>
                                      ...
                                      >
                                        <a
                                          onClick={() =>
                                            ...
                                            this.setState({
                                              agentTableFilters: [
                                                { field: 'q', value: 'status=disconnected' },
                                              ],
                                            })
                                          }
                                        >
                                      ...
                                      >
                                        <a
                                          onClick={() =>
                                            ...
                                            this.setState({
                                              agentTableFilters: [
                                                { field: 'q', value: 'status=never_connected' },
                                              ],
                                            })
                                          }
                                        >
