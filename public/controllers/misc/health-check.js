...
class HealthCheck {
    constructor(
        ...
        genericReq, apiReq, appState, testAPI, 
        errorHandler, wazuhConfig, Private, $window
    ){
        this.$scope         = $scope;
    ...
    handleError(error) {
        ...
        this.$scope.errors.push(this.errorHandler.handle(error,'Health Check',false,true));
    }

    async checkPatterns() {
        try {
            const data         = await this.savedObjectsClient.get('index-pattern', this.appState.getCurrentPattern());
            const patternTitle = data.attributes.title;

            if(this.checks.pattern) {
                const i = this.$scope.results.map(item => item.id).indexOf(2);
                const patternData = await this.genericReq.request('GET', `/api/wazuh-elastic/pattern/${patternTitle}`);
            ...
            if(this.checks.template) {
                ...
                const templateData = await this.genericReq.request('GET', `/api/wazuh-elastic/template/${patternTitle}`);
                if (!templateData.data.status) {
                    this.$scope.errors.push("No template found for the selected index-pattern.");
                    this.$scope.results[i].status = 'Error';
                } else {
                    this.$scope.processedChecks++;
                    this.$scope.results[i].status = 'Ready';
    ...
    async checkApiConnection() {
        try {
            if(this.checks.api) {
                const data = await this.testAPI.checkStored(JSON.parse(this.appState.getCurrentAPI()).id);
                if(data && data.data && data.data.idChanged) {
                    const apiRaw = JSON.parse(this.appState.getCurrentAPI());
                    this.appState.setCurrentAPI(JSON.stringify({name: apiRaw.name, id: data.data.idChanged }));
                }
                ...
                if (data.data.error || data.data.data.apiIsDown) {
                    ...
                    if(this.checks.setup) {
                        const versionData = await this.apiReq.request('GET', '/version', {});
                        const apiVersion  = versionData.data.data;
                        const setupData   = await this.genericReq.request('GET', '/api/wazuh-elastic/setup');
                        ...
                        if (apiSplit[0] !== appSplit[0] || apiSplit[1] !== appSplit[1]) {
                            this.$scope.errors.push("API version mismatch. Expected v" + setupData.data.data["app-version"]);
                            this.$scope.results[i].status = 'Error';
                        } else {
                            this.$scope.processedChecks++;                            
                            this.$scope.results[i].status = 'Ready';
    ...
    async load() {
        try {
            ...
            this.$scope.results.push({ id:0,description: 'Check Wazuh API connection',status: this.checks.api ? 'Checking...' : 'disabled' });
            this.$scope.results.push({ id:1,description: 'Check for Wazuh API version',status: this.checks.setup ? 'Checking...' : 'disabled' });
            this.$scope.results.push({ id:2,description: 'Check Elasticsearch index pattern',status: this.checks.pattern ? 'Checking...' : 'disabled' });
            this.$scope.results.push({ id:3,description: 'Check Elasticsearch template',status: this.checks.template ? 'Checking...' : 'disabled'});

            for(let key in this.checks) this.$scope.totalChecks += (this.checks[key]) ? 1 : 0;
