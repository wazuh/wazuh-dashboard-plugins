...
const helpLinks = [
  { text: 'Checking connection with manager', href: 'https://documentation.wazuh.com/current/user-manual/agents/agent-connection.html' },
  { text: 'Client reference', href: 'https://documentation.wazuh.com/current/user-manual/reference/ossec-conf/client.html' }
];

const mainSettings = [
  { field: 'crypto_method', label: 'Method used to encrypt communications' },
  { field: 'remote_conf', label: 'Remote configuration is enabled' },
  { field: 'auto_restart', label: 'Auto-restart the agent when receiving valid configuration from manager' },
  { field: 'notify_time', label: 'Time (in seconds) between agent checkings to the manager' },
  { field: 'time-reconnect', label: 'Time (in seconds) before attempting to reconnect' },
  { field: 'config-profile', label: 'Configuration profiles' },
  { field: 'local_ip', label: 'IP address used when the agent has multiple network interfaces' }
];
...
class WzConfigurationClient extends Component{
  ...
  render(){
    ...
    return (
      <Fragment>
        {currentConfig['agent-client'] && isString(currentConfig['agent-client']) && (
          <WzNoConfig error={currentConfig['agent-client']} help={helpLinks}/>
        )}
        {currentConfig['agent-client'] && !isString(currentConfig['agent-client']) && (
