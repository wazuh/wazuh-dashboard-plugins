...
export function getApiRequestJsonModelfromText(text) {
  ...
  allLines.forEach(function (line) {
    ...
    var exp = line.split(" ");
    endpointObj.name = exp[1];
    var endpointSplitted = exp[1].split("/");
    endpointSplitted.forEach(function (element) {
      if (element.startsWith(":")) endpointObj.args.push({
        "name": element
      })
    });
    var methodExists = json.find(function (item) {
      return item.method == exp[0]
    });
    if (methodExists) {
      methodExists.endpoints.push(endpointObj);
    } else {
      json.push({
        "method": exp[0],
        "endpoints": [
